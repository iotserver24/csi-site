...
const Github = ({ size = 16, ...props }: IconProps) => (
...
const Twitter = ({ size = 16, ...props }: IconProps) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor" aria-hidden {...props}>
...
const Footer: React.FC = () => {
  ...
  return (
    <footer className="bg-white dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800">
      <div className="w-full max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 py-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <div className="flex items-center gap-2.5 mb-6">
            <div className="w-9 h-9 bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg flex items-center justify-center shadow-md shrink-0">
               {/* eslint-disable-next-line @next/next/no-img-element */}
               <img src="/csi-logo.png" alt="CSI" className="w-5 h-5" />
            </div>
            <div>
              <h3 className="text-lg font-bold text-gray-900 dark:text-white">CSI NMAMIT</h3>
              <p className="text-xs text-gray-500 dark:text-gray-400">Computer Society of India</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            {socialLinks.map(({ icon: Icon, href, label }) => (
              <motion.a key={label} href={href} target="_blank" rel="noopener noreferrer"
                        ...
                        className={`w-8 h-8 rounded-full flex items-center justify-center text-gray-600 dark:text-gray-400
