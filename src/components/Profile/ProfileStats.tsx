...
const ProfileStats: React.FC<Props> = ({ eventsCount: eventsCountProp }) => {
  const { user } = useAuth()

  const certificatesCount =
    (user?.certificates && Array.isArray(user.certificates) && user.certificates.length) || 0

  const awardsCount =
    (user?.awards && Array.isArray(user.awards) && user.awards.length) || 0

  const eventsCount =
    typeof eventsCountProp === 'number'
      ? eventsCountProp
      : (user?.events && Array.isArray(user.events) && user.events.length) ||
        (user?.participation && Array.isArray(user.participation) && user.participation.length) ||
        0
