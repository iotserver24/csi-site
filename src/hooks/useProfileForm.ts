...
export const useProfileForm = () => {
  ...
  const [profileData, setProfileData] = useState<ProfileFormData>({ name: '', email: '', phone: '', college: 'NMAMIT', branch: '', year: '', bio: '', usn: '', github: '', linkedin: '' })
  ...
  const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => setProfileData(prev => ({ ...prev, [event.target.name]: event.target.value }))
