...
import { membershipPlans } from '../../../../src/db/schema'
import { jsonError, requireUser } from '../../../../src/lib/server-auth'

const fallbackPlans: Record<string, { price: number; name: string }> = { 'one-year': { price: 358, name: '1-Year Executive Membership' }, 'two-year': { price: 664, name: '2-Year Executive Membership' }, 'three-year': { price: 919, name: '3-Year Executive Membership' } }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const plan = dbPlan || fallbackPlans[planId as string]
    if (!plan) return NextResponse.json({ error: 'Invalid plan selected' }, { status: 400 })
    if (context.user.membershipStatus === 'active' && context.user.membershipExpiresAt && context.user.membershipExpiresAt > new Date()) return NextResponse.json({ error: 'Active membership already exists' }, { status: 400 })
