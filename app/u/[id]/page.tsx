...
import { ArrowLeft, Calendar, Award, Copy, Check } from 'lucide-react'
import { api } from '../../../src/lib/api-client'
import { getPlanDisplayLabel } from '../../../src/data/membershipData'
...
function extractUsername(url: string, platform: 'github' | 'linkedin'): string {
  try {
    const u = new URL(url)
...
export default function SharedProfilePage() {
  ...
  const planLabel = getPlanDisplayLabel(user.membershipType)
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row items-center md:items-start gap-6 mb-10">
          <div className="relative">
            <Image src={user.photoURL || '/default-avatar.svg'} alt={user.name} width={120} height={120} unoptimized
              className={`w-28 h-28 rounded-full object-cover ring-4 ${
                isMember ? 'ring-yellow-400/70' : 'ring-gray-200 dark:ring-gray-800'
              }`} />
            {isMember && (
          ...
          <div className="text-center md:text-left flex-1">
            <div className="flex flex-col md:flex-row items-center gap-3 mb-2 flex-wrap">
              ...
              <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 capitalize">{role}</span>
            </div>
            {isMember && (
              <p className="text-sm text-yellow-700/80 dark:text-yellow-400/80 mb-1 font-medium">
